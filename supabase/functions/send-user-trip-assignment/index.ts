  ...
  <body style="margin:0;background:#fbf9f4;color:#11192e;font-family:Arial,sans-serif;">
    <div style="max-width:720px;margin:0 auto;padding:28px 18px;">
      ...
      <div style="background:#ffffff;border:1px solid #ece7da;border-top:0;border-radius:0 0 18px 18px;padding:28px;">
        ...
        <p style="margin:0 0 18px;font-size:14px;line-height:1.6;color:#5f6678;">Please feel free to contact CTSC Travel for any inquiries.</p>
      </div>
    </div>
  </body>
</html>`;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const customerEmail = (booking.guest_email ?? userEmail)?.trim();
    if (!customerEmail) return jsonResponse({ error: "Booking customer does not have an email address" }, 400);
    ...
    const dashboardUrl = `${resolveSiteUrl(req).replace(/\/$/, "")}/dashboard`;
    ...
    const rows: Array<[string, string | null | undefined]> = [
      ...
      ["Trip direction", hasReturn ? "Return trip" : "One-way"],
    ...
    console.error("send-user-trip-assignment failed:", err);
